...
const Tx = {
	encode(message, writer = new _bufbuild_protobuf_wire.BinaryWriter()) {
		for (const v of message.messages) require_any.Any.encode(v, writer.uint32(10).fork()).join();
	...
	decode(input, length) {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					if (tag !== 10) break;
					message.messages.push(require_any.Any.decode(reader, reader.uint32()));
					continue;
				case 2:
					if (tag !== 18) break;
	...
	fromJSON(object) {
		return {
			messages: globalThis.Array.isArray(object?.messages) ? object.messages.map((e) => require_any.Any.fromJSON(e)) : [],
	...
	toJSON(message) {
		const obj = {};
		if (message.messages?.length) obj.messages = message.messages.map((e) => require_any.Any.toJSON(e));
	...
	fromPartial(object) {
		const message = createBaseTx();
		message.messages = object.messages?.map((e) => require_any.Any.fromPartial(e)) || [];
	...
	decode(input, length) {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					if (tag !== 10) break;
					message.pub_key = require_any.Any.decode(reader, reader.uint32());
					continue;
				case 2:
					if (tag !== 18) break;

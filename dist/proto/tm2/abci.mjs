...
const ResponseBase = {
	encode(message, writer = new BinaryWriter()) {
		if (message.error !== void 0) Any.encode(message.error, writer.uint32(10).fork()).join();
		if (message.data.length !== 0) writer.uint32(18).bytes(message.data);
		for (const v of message.events) Any.encode(v, writer.uint32(26).fork()).join();
	...
	toJSON(message) {
		const obj = {};
		if (message.error !== void 0) obj.Error = Any.toJSON(message.error);
		if (message.data !== void 0) obj.Data = base64FromBytes(message.data);
		if (message.events?.length) obj.Events = message.events.map((e) => Any.toJSON(e));
